...
brave.runtime.onInstalled.addListener(async () => {
  ...
  // runtime.
  let url = brave.runtime.getURL('hello.html')
  ...
  let tab = await brave.tabs.create({ url })
...
brave.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (changeInfo.status === 'complete') {
        console.log('Tab updated, attempting to inject content script');
        // First try to inject CSS
        brave.tabs.insertCSS(tabId, {
            file: 'style.css'
        }, function() {
            if (brave.runtime.lastError) {
                console.error('Error injecting CSS:', brave.runtime.lastError);
            } else {
        ...
        brave.tabs.executeScript(tabId, {
            file: 'contentScript.js'
        }, function() {
            if (brave.runtime.lastError) {
                console.error('Error injecting content script:', brave.runtime.lastError);
            } else {
